...
//Método que salva um cliente dentro do banco de dados 
    public void Salvar(ModelCliente c) {

        Connection con = ConnectionFactory.getConnection();
        PreparedStatement stmt = null;

        try {
            stmt = con.prepareStatement("INSERT INTO tbl_cliente(nomeCliente, cpfCliente, endCliente, baiCliente, cidCliente, ufCliente, cepCliente, telCliente, emCliente)VALUES(?,?,?,?,?,?,?,?,?)");
...
//Mostra uma lista de clientes cadastrados        
    public List<ModelCliente> Leitura() {

        Connection con = ConnectionFactory.getConnection();
        PreparedStatement stmt = null;
        ResultSet rs = null;

        List<ModelCliente> objclientes = new ArrayList<>();
...
//Método de alterar um cliente    
    public void Alterar(ModelCliente c) {

        Connection con = ConnectionFactory.getConnection();
        PreparedStatement stmt = null;

        try {
            stmt = con.prepareStatement("UPDATE tbl_cliente SET nomeCliente = ?, cpfCliente = ?, endCliente = ?, baiCliente = ?, cidCliente = ?, ufCliente = ?, cepCliente = ?, telCliente = ?, emCliente = ? WHERE idCliente = ?");
            stmt.setString(1, c.getNomeCliente());
            stmt.setString(2, c.getCpfCliente());
            stmt.setString(3, c.getEndCliente());
            stmt.setString(4, c.getBaiCliente());
...
//Exclui um cliente do banco de dados
    public void Excluir(ModelCliente c) {
...
//Método de pesquisa de um cliente por nome    
    public List<ModelCliente> Pesquisar(String nomeCliente) {
